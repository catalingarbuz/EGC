...
void Lab7::Update(float deltaTimeSeconds)
{
...

    {
        ...
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, glm::vec3(0, 0, 0.8f));
    }
    ...
    // Render ground
    {
        ...
        RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, glm::vec3(0.9f, 0.9f, 0.9f));
    }
...
void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    GLint camPos = glGetUniformLocation(shader->program, "eye_position");
    glUniform3fv(camPos, 1, glm::value_ptr(eyePosition));
    // TODO(student): Set material property uniforms (shininess, kd, ks, object color)
    GLint matShininess = glGetUniformLocation(shader->program, "material_shininess");
    glUniform1i(matShininess, materialShininess);

    GLint matKd = glGetUniformLocation(shader->program, "material_kd");
    glUniform1f(matKd, materialKd);

    GLint matKS = glGetUniformLocation(shader->program, "material_ks");
    glUniform1f(matKS, materialKs);

    GLint objColorPos = glGetUniformLocation(shader->program, "object_color");
    glUniform3fv(objColorPos, 1, glm::value_ptr(color));
