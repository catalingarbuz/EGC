...
/* Ex 7
   Cand modific filtrele de la GL_TEXTURE_MAG_FILTER, identific o pixelare mai mare cand ne apropiem 
   de obiect pentru GL_NEAREST, si o pixelare mai mica la apropiere pentru GL_LINEAR.
   La GL_TEXTURE_MIN_FILTER, diferentele principale le identific la GL_NEAREST cu GL_LINEAR, si
   la GL_NEAREST_MIPMAP_NEAREST cu GL_LINEAR_MIPMAP_LINEAR, in ambele cazuri, cu filtrul LINEAR, 
   indepartarea de obiecte si modificarea texturilor se face mai smooth, iar in cazul cu NEAREST, 
   avem o imagine putin mai confuza la indepartarea de obiecte.*/
...
void Lab9::Update(float deltaTimeSeconds)
{
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(-1, 0.5f, -1));
        modelMatrix = glm::scale(modelMatrix, glm::vec3(0.4f));
       // modelMatrix = glm::rotate(modelMatrix, RADIANS(25.0f), glm::vec3(1, 1, 0));
        RenderSimpleMesh(meshes["quad"], shaders["LabShader"], modelMatrix, mapTextures["crate"], mapTextures["grass"]);
    }
}
...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
    ...
    int loc_is_quad = glGetUniformLocation(shader->program, "is_quad");
    glUniform1i(loc_is_quad, is_quad);
